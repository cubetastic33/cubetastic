firebase.auth().onAuthStateChanged(function(user) {
  if (user) {
    ...
    db.ref('users/' + uid).on('value', function(data) {
      ...
      $('#userPhoto').click(function() {$('#updateProfilePic').click();});
...
function resetReauthenticationDialog() {
  ...
  $('#reauthenticateBody').html('\
    You are attempting to change sensitive data which is currently locked. To unlock it, please\
    reauthenticate by entering your password below:\
    <div class="outlined-input-field" style="margin-top: 1rem">\
      <i class="material-icons">lock</i>\
      <input type="password" id="password" class="outlined-text-field" autocomplete="off" readonly onfocus="if (this.hasAttribute(\'readonly\')) {this.removeAttribute(\'readonly\');this.blur();this.focus();}">\
      <label for="password" style="background-color: #FFFFFF">Password</label>\
    </div><br>\
    <button id="reauthenticateButton" class="mdc-button mdc-button--raised">Reauthenticate</button>\
  ');
  $('#password').focusin(function() {
    $('#password ~ label').animate({
      'fontSize': '0.8rem',
...
dialog.listen('MDCDialog:cancel', resetReauthenticationDialog);
    ...
    if ($('#password').val() != '') {
      ...
      firebase.auth().currentUser.reauthenticateAndRetrieveDataWithCredential(credential).then(function() {
        ...
        $('#reauthenticateBody').html('\
          ...
          <div class="outlined-input-field" style="margin-top: 1rem">\
            <i class="material-icons">lock</i>\
            <input type="password" id="newPassword" class="outlined-text-field" autocomplete="off" readonly onfocus="if (this.hasAttribute(\'readonly\')) {this.removeAttribute(\'readonly\');this.blur();this.focus();}">\
            <label for="newPassword" style="background-color: #FFFFFF">Password</label>\
          </div><br>\
          <button id="changePassword" class="mdc-button mdc-button--raised">Change Password</button>\
        ');
  ...
  $('#reauthenticateButton').click(function() {
    if ($('#password').val() != '') {
      ...
      firebase.auth().currentUser.reauthenticateAndRetrieveDataWithCredential(credential).then(function() {
        snackbar.show({message: 'Successfully Reauthenticated!', timeout: 4000});
        $('#reauthenticateBody').html('\
          Enter your updated email address below:\
          <div class="outlined-input-field" style="margin-top: 1rem">\
            <i class="material-icons">email</i>\
            <input type="email" id="email" class="outlined-text-field" autocomplete="off">\
            <label for="email" style="background-color: #FFFFFF">Email</label>\
          </div><br>\
          <button id="updateEmail" class="mdc-button mdc-button--raised">Update Email</button>\
        ');
